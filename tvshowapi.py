...
def FileNamesPrepare(filename):
	...
	urls = [r's(\d+)e(\d+)', r's(\d+) e(\d+)', r'(\d+)[x|-](\d+)', r'E(\d+)', r'Ep(\d+)', r'\((\d+)\)', r'S(\d+)e(\d+)', r'S(\d+)E(\d+)', r'Сезон (\d+)Эпизод (\d+)', r'(\d+).sezon.(\d+).serija', r'_(\d+)_(\d+)', r'(\d+)_', r'(\d+) ', r'.(\d+).', r'(\d+). ', r'(\d+).iz', r'_(\d+)', r'.(\d+)', r'(\d+) of']
	for file in urls:
