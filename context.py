...
class MyWindow(pyxbmct.AddonDialogWindow):
	...
	def __init__(self, title, settings, links):
		...
		if kodi_ver_major < 16:
			...
			li.setProperty('link', 'plugin://script.media.aggregatorfork/?action=settings')
			self.list.addItem(li)

			li = xbmcgui.ListItem(u'СМОТРИТЕ ТАКЖЕ...')
			li.setProperty('link', 'plugin://script.media.aggregatorfork/?action=show_similar')
			self.list.addItem(li)

			li = xbmcgui.ListItem(u'ПОИСК ИСТОЧНИКОВ...')
			li.setProperty('link', 'plugin://script.media.aggregatorfork/?action=add_media')
			self.list.addItem(li)
			...
			if filesystem.exists(pathUnited.decode('utf-8')):
				...
				li.setProperty('link', 'plugin://script.media.aggregatorfork/?action=united_search')
				self.list.addItem(li)
...
def main(settings=None, path=None, name=None, run=None):
	...
	if link == 'plugin://script.media.aggregatorfork/?action=settings':
		xbmc.executebuiltin('Addon.OpenSettings(script.media.aggregatorfork)')
		del window
		return True

	if link == 'plugin://script.media.aggregatorfork/?action=show_similar':
		from context_show_similar import show_similar
		if show_similar():
			del window
			return True

	if link == 'plugin://script.media.aggregatorfork/?action=add_media':
		from context_get_sources import get_sources
		get_sources(settings)
		return True

	if link == 'plugin://script.media.aggregatorfork/?action=united_search':
